<?php if (isset($listaDocentes) && !empty($listaDocentes)): ?>

<?php foreach ($listaDocentes as $key => $value): ?>

<div class="media well-white bloque-top marcador-borde-azul" style="margin: 0px;">
	<a class="pull-left" href="#">
  		<img class="media-object img-rounded" data-src="holder.js/64x64" alt="64x64" style="width: 40px; height: 40px;" src="data:image/png;base64,">
	</a>
	<div class="media-body">
   		<div class="col-md-3">
   			
   			<a 
				id="#<?php //echo $value['id_time']; ?>" 
				class=""
				href=#"<?php //echo site_url('foro/foroModal/' . $value['Materia_id'] . '/' . $numeroClase . '/' . $value['id_time']); ?>" title="" 
				<?php //data-target="#modalPerfil" data-toggle="modal" ?>
				>
				<strong class="media-heading text-success"><?php echo $value['nombre'] .' '. $value['apellido']; ?></strong>
			</a>
    			<br><a class="fontSize1 hidden" href="#"><i class="fa fa-envelope-alt"></i> Mensaje</a>
    			<!-- <a class="fontSize1 padding1" href="#"><i class="fa fa-twitter"></i> Tweet</a> -->
   		</div>
				    							
		<div class="col-md-5">
			<div class="row">
				<div class="col-md-6"><span class="text-muted"><?php echo $value['tipo_identificacion'] ?>: </span><?php echo $value['identificacion'] ?></div>
				<div class="col-md-6"><span class="text-muted">Profesión: </span><?php echo $value['profesion'] ?></div>
			</div>
		</div>

		<div class="col-md-4" style="text-align: right;">
			<div class="row hidden">
				<div class="btn-group">
					<button class="btn btn-info btn-lg"><i class="fa fa-edit"></i> </button>
					<button class="btn btn-warning btn-lg"><i class="fa fa-arrow-down"></i> </button>
					<button class="btn btn-danger btn-lg"><i class="fa fa-times"></i> </button>
				</div>
			</div>
		</div>
	</div>

</div>

<?php endforeach ?>

<?php else: ?>
	<p>No se han encontrado resultados para tu búsqueda </p>
<?php endif; ?>